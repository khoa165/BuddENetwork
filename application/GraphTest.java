...
class GraphTest {
  ...
  void test003_AddDuplicateVertex() {
    try {
      ...
      // DuplicateUserException should be caught here, since duplicate user was
      // added
    } catch (IllegalNullArgumentException e) {
      fail("Cannot add a vertex for this user; the user is null.");
    } finally {
  ...
  void test004_RemoveVertex_in_graph() {
    try {
      ...
      fail(
          "The User harry is not already in the graph, so cannot remove vertex.");
    } finally {
      // Vertex harry in graph should have been removed, so graph order should
      // be 0
      assertEquals(graph.order(), 0);
    }
  }
  ...
  void test005_RemoveVertex_not_in_graph() {
    try {
      ...
      // UserNotFoundException should be caught here, since kenny is not already
      // in graph
    } finally {
      assertEquals(graph.order(), 1);
    }
  ...
  void test007_AddValidEdge() {
    try {
      ...
      fail(
          "DuplicateFriendshipException thrown, but this should not be a duplicate friendship.");
    } finally {
  ...
  void test008_AddDuplicateEdge() {
    try {
      ...
      // DuplicateFriendshipException should be caught here, since harry and
      // kenny are friends
    } finally {
      assertEquals(graph.size(), 1);
    }
  ...
  void test009_AddNullEdge() {
    try {
      ...
      // IllegalNullArgumentException should be caught here, since one vertex of
      // edge is null
    } catch (DuplicateFriendshipException e) {
  ...
  void test012_RemoveNullEdge_from_graph() {
    try {
      ...
      // IllegalNullArgumentException should be caught here, since one vertex in
      // edge is null
    } catch (DuplicateFriendshipException e) {
      fail("DuplicateFriendshipException thrown.");
    } catch (FriendshipNotFoundException e) {
  ...
  void test013_RemoveEdge_not_in_graph() {
    try {
      ...
      // FriendshipNotFoundException should be caught here, since edge does not
      // exist in graph
    } catch (UserNotFoundException e) {
      fail("UserNotFoundException thrown.");
    } finally {
  ...
  void test014_RemoveEdgeWithUser_not_in_graph() {
    try {
      ...
      // UserNotFoundException should be caught here, since one user does not
      // exist in graph
    } finally {
      assertEquals(graph.order(), 3);
      assertEquals(graph.size(), 1);
  ...
  void test016_GetNeighbors_of_vertex_in_graph() {
    ...
    try {
      ...
      if (!correctAdj.equals(allAdj)) {
        fail(
            "Correct adjacent vertices list for the grpah is not returned from method. "
                + "Correct list is " + correctAdj + ", but returned " + allAdj);
      }
    }
  }
  ...
  void test017_GetNeighbors_of_null_vertex_graph() {
    ...
    try {
      ...
      // IllegalNullArgumentException should be caught here, since vertex is
      // null
    } catch (DuplicateFriendshipException e) {
      fail("DuplicateFriendshipException thrown.");
    } catch (UserNotFoundException e) {
  ...
  void test018_GetNeighbors_of_vertex_not_in_graph() {
    ...
    try {
      ...
      // UserNotFoundException should be caught here, since shannon is not user
      // in graph
    } finally {
      if (!correctAdj.equals(allAdj)) {
        fail(
